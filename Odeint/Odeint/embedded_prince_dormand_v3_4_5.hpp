...
std::vector<T> Runge_Kutta2(const F& f, const T& x0, std::vector<T>& y, const T& h);

template<typename F, typename T>
void Embedded_Prince_Dormand_v3_4_5(const F& f, const T& x, std::vector<T>& y, const T& h, T& h2) {

	auto k = Runge_Kutta2(f, x, y, h);
	k = Runge_Kutta2(f, x, k, h2);
